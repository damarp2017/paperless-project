...
class OrderController extends Controller
{
    ...
    {
        $data = $request->all();
        $seller_store = Store::where('id', $request->sell_by_store)->first();
        if (isOwner($seller_store) || isStaff($seller_store) || isCashier($seller_store)) {
            ...
            if ($request->has('buy_by_user')) {
                ...
                if ($owner_buyer_store->fcm_token != null) {
                    $notif = new Notification();
                    $notif->sender = $store->id;
